...
| 章节 ID | 文件 | 标题 | 状态 | 最后核实 |
| --- | --- | --- | --- | --- |
| prep-qa50 | 实战包.html | 对客交锋 95 题（8 维度 + ★必背 24 + 4 兜底话术，每题深潜直达网页版章节；交锋=面试的隐喻） | ✅ | 2026-07-22 |
| prep-path | 学习路径.html | 三条路线：七天通读 / 对客前 30 分钟 / 交锋前夜（★必背打头 + 框架背诵） | ✅ | 2026-07-22 |
| prep-map | 全库一页纸.html | 七层书架 × 21 模块（页数实测，共 1998 页）+ 五条串联主动脉 + 保鲜看板 | ✅ | 2026-07-22 |
...
| 本产物 | 指向 | 关系 |
| --- | --- | --- |
| prep-qa50 | （全库 21 册网页版章节锚点） | 每题的完整版答案与出处在源模块对练页；本包只是电梯版索引。2026-07-22 起 137 条深潜链接全部直达网页版章节，由 `check_prep_coverage.py` 逐条校验 |
| prep-map 保鲜看板 | mcp / model-landscape / multimodal | 已排定的三个定点复查（07-28 / 08-31 / 08 上旬）集中可见 |
| prep-map 书架表 | （全库 21 模块；新增两册暂只有讲义面） | 模块名 → 网页版模块页；页数徽标 → PPT 讲义 README（页数是 PPT 面的属性） |
...
「先记上以后再说」。2026-07-22 起该文件为空：21 个模块全部有题。
