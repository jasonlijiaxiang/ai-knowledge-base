...
| 托管商用模型 ≥1M 上下文窗口已成 2026 旗舰标配（Gemini 3.1 Pro 1M；开源 Llama 4 Scout 标称 10M）；RULER/MRCR v2/NoLiMa 显示多事实检索过 200K 普遍掉 30–60 分 | llm-inference-kv | 2026-09-05 | ai.google.dev Gemini 3.1 Pro 模型页（1M）；ai.meta.com Llama 4 博客（10M）；genai.club 长上下文基准汇总（掉分区间）（2026-09-05 逐字核对） | — | B | 30 | 跨模型汇总的掉分区间；「13 家」这类跨站计数随汇总口径漂移，已删精确家数，单模型有效长度看当期基准 |
