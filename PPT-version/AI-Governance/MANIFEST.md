...
| 建立日期 | 2026-08-02 |
| 最后更新 | 2026-08-02 A 类新增（Codex 对照库借鉴第 5 步）：方案见 `_maintenance/2026-08-02-两个新模块-方案.md`，逐章内容联网核实后写入；讲义 117 页（生成器 kb_deck_build.py 渲染，audit 十七项 PASS、LibreOffice 转 PDF 无错、逐页渲染目检抽验封面/导览/章节过渡/学习目标/流程页/五列云落点表）；时效性事实 43 条均为本次联网核实；2026-08-02（同日晚）网页版落地 `Web-version/ai-governance/index.html`：15 节（11 章 + 上云怎么落地 + 现场高频追问 + 相关模块 + 来源），覆盖与缺口矩阵存 `raw-data/2026-08-02-网页版覆盖与缺口矩阵.md`；交互件＝用途分诊器（附件三类别／画像／自主度／权利影响／法域 → 组织风险档 + 法律分类 + 该走哪几步 + 证据包必备件，结论一律标明这是分诊不是合规意见）；事实级 0、缺口级 0 |
| 产出 skill 版本 | v8.7 |
