...
| Azure API Management 原生内置按 token 限流（llm-token-limit 策略族）+ LLM 专属策略，并有统一模型 API（Unified Model API，preview：单一 OpenAI 兼容端点接多后端；后端格式现仅 OpenAI Chat Completions 与 Anthropic Messages 两种）与语义缓存（llm-semantic-cache + Azure Managed Redis）；「唯一原生」不成立——Google Apigee 原生自带 PromptTokenLimit / LLMTokenQuota LLM token 策略与语义缓存、Model Armor、动态模型路由，官方文档设专门「AI gateway capabilities」页（2026-07-13 更新） | gw-cost | 2026-09-05 | learn.microsoft.com genai-gateway-capabilities；docs.cloud.google.com/apigee ai-capabilities（2026-09-05 逐字核对） | — | B | 30 | 排他断言（「唯一」）必须逐云核对官方策略清单；三家月级演进 |
...
| 云厂商格局（2026-09-05 复核）：Cloudflare AI Gateway 已不止「花费上限+缓存」——单一 OpenAI 兼容端点接 20+ 家 provider，另有 rate limiting、Guardrails(Beta)、DLP(Beta)、动态路由(Beta)、统一计费；Google Apigee 官方设专门「AI gateway capabilities」文档页（Model Armor、语义缓存、LLM token 策略、动态模型路由、MCP 工具化），「有部分能力」低估；AWS 以 Bedrock 为主，Bedrock 已提供 OpenAI 兼容 API 族（Chat Completions / Responses）并上架 OpenAI GPT-5.5/5.4 等第三方模型 | gw-cheatsheet | 2026-09-05 | developers.cloudflare.com/ai-gateway/usage/chat-completion/；docs.cloud.google.com/apigee ai-capabilities；docs.aws.amazon.com/bedrock models-api-compatibility（2026-09-05 逐字核对） | — | B | 30 | 格局快照月级演进，引用前重开官方 features/capabilities 页；否定式能力判断抓到反证即作废 |
...
| Kong AI Gateway 高级插件（AI Proxy Advanced / Semantic Cache 等）标注 tier: ai_gateway_enterprise,需企业订阅;开源版仅基础 AI Proxy | gw-cheatsheet | 2026-09-05 | developer.konghq.com/plugins/ai-proxy-advanced/reference.md（frontmatter 逐字；2026-09-05） | — | A | 90 | 授权层级随版本调整，不能据此推断开源版长期缺口；服务名可能改名，改名不改本质 |
...
| AWS Multi-Provider Generative AI Gateway 为 AWS Solutions Library 官方 Guidance（GitHub: aws-solutions-library-samples/guidance-for-multi-provider-generative-ai-gateway-on-aws）：LiteLLM + ECS/EKS 的 Terraform 自部署参考架构，非托管网关产品；AWS 托管侧统一多模型 API 现状见上行（Bedrock 提供 OpenAI 兼容 API 族） | gw-cheatsheet | 2026-09-05 | AWS 官方 GitHub 仓 github.com/aws-solutions-library-samples/guidance-for-multi-provider-generative-ai-gateway-on-aws README（2026-09-05 逐字核对；Solutions 页正文 JS 渲染抓不到） | — | B | 30 | Guidance 状态以官方仓为准；「AWS 无托管统一多模型网关」这类否定式观察不登记 |
