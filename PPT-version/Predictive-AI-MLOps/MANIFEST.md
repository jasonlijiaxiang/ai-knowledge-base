...
| 建立日期 | 2026-08-02 |
| 最后更新 | 2026-08-02 A 类新增（Codex 对照库借鉴第 5 步）：方案见 `_maintenance/2026-08-02-两个新模块-方案.md`，逐章内容联网核实后写入；讲义 122 页（生成器 kb_deck_build.py 渲染，audit 十七项 PASS、LibreOffice 转 PDF 无错、逐页渲染目检抽验封面/导览/章节过渡/学习目标/流程页/五列云落点表）；时效性事实 30 条均为本次联网核实；2026-08-02（同日晚）网页版落地 `Web-version/predictive-ai-mlops/index.html`：14 节（10 章 + 上云怎么落地 + 23 道现场高频追问 + 相关模块 + 来源），覆盖与缺口矩阵存 `raw-data/2026-08-02-网页版覆盖与缺口矩阵.md`；交互件＝「告警响了，先别急着重训」分诊器（七类信号 → 第一动作 → 五步分诊落点 → 该不该重训，判据全取自第 7 章，六场景已对已知答案）；事实级 0、缺口级 0（讲义按讲义标准完备，网页更细的部分不构成讲义缺口） |
| 产出 skill 版本 | v8.7 |
...
| 预测式监控的云上落点（服务名快照）：Azure ML 表格数据内建监控信号、Vertex AI 模型监控、AWS SageMaker Model Monitor（已不再向新客户开放，官方改荐开源自建）；开源侧代表为 Evidently（漂移检测）与 NannyML（无真值期性能估计） | pam-monitoring | 2026-08-02 | Microsoft Learn《Model monitoring in production - Azure Machine Learning》；AWS 文档 sagemaker/latest/dg/model-monitor.html 与 model-monitor-availability-change.html；Google Cloud Vertex AI 模型监控文档；Evidently 与 NannyML 官方文档（均 2026-08-02 查见） | — | A | 30 | 是当日的服务名快照，不代表三家云能力对等，也不代表形态不变 |
| 两种 AI 的云上门面（服务名快照）：注册与实验为 SageMaker 托管 MLflow、Vertex AI 模型注册表、Azure ML 注册表；生成式侧另有 Bedrock、Vertex AI Studio、Microsoft Foundry；在线服务为 SageMaker 端点、Vertex 端点、Azure ML 在线端点 | pam-platform | 2026-08-02 | Microsoft Learn《What is Azure Machine Learning?》（页面 updated 2026-03-30）；AWS SageMaker 与 Bedrock 产品文档；Google Cloud Vertex AI 产品文档（均 2026-08-02 查见） | — | A | 30 | 是当日的服务名快照，产品名与形态月级变动，不代表三家的切分方式相同 |
