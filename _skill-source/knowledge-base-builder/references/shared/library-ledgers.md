...
就是"动了真源、漏刷派生"（事故清单与原则在 `core-rules.md`，此处不复写）。各任务分支的回刷清单（ppt-rules 增补回刷、illustration-rules
...
| 整库版本戳 | KB-CONFIG「库版本」 | 各页右上角 `.ver`、Release 标签 | 发整库版本 | `stamp_kb_version.py --check` + `check_version_tag.py` |
| 可派生数目（模块/网页覆盖/门禁道数） | MANIFEST 目录 / 网页目录 / `run_all_gates.sh` gate 行 | CLAUDE.md、README、开始使用、`_prep`、`维护手册` 等活面 | 增删模块、增删门禁 | `check_derived_counts.py` |
...
| 层级色相 | `.hue-N` 的 `--hue`/`--hue-bg` | 紫 `#7C3AED`、蓝 `#2563EB`、青 `#0891B2`、橙 `#EA580C`、绿 `#16A34A`、靛 `#6366F1`、琥珀 `#D97706`——知识地图按层取色，用于模块字标块、层标点、侧栏色点（导航站的视觉节奏来源）；**色相数 ≥ 层数即可，层多时循环取色** |
