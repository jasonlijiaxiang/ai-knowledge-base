...
1. **识别输入 → 路由 → 存档。** 先分析本次输入（概念/名词/对话/资料），提炼出它到底在讲哪个主题；
   ...
   **输入指向库内已有参考资料时不复制大文件**：若输入是根目录 `_reference/` 下的对照成品
   ...
   要不要顺便体检"——保鲜机制的零成本兜底。别再手工比日期，判据以脚本为准。
...
- `shared/core-rules.md` — **核心原则（横切原则的唯一出处）**：真源唯一派生只引用、内容决定
  形式不设配额、分层验收防线模型、工具中立、错误转化系统能力、**深度标尺**（定义→机制→边界→
  判断→证据五步 + 理论深度门槛：公式只在会改变读者判断时进正文）（拿不准某条铁律的权威出处时先查这里）。
- `shared/failure-matrix.md` — **错题本（人工验收清单的唯一出处）**：过去出过什么事、现在由
  哪道闸拦、**机器拦不住的那一半复核时该问哪一句**。分两节——「已被机器判死」按脚本组织，
  作用是**划走注意力**（那些不必肉眼再找一遍）；「只能靠人」按六个面给出可回答的问句。
  它是 `core-rules` §五第 ④ 条「无法自动判断的部分加入人工验收清单」的落点；在它之前那条
  一直只是口号。收尾验收、全面 review、第 6 步自省时读，日常任务不必通读。
